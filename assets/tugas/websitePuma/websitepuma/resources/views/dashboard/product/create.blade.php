...
@section('konten')
    <div class="pb-3"><a href="{{route('product.index')}}" class="btn btn-secondary">
        << kembali </a>
    ...
    <form action="{{route('product.store')}}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="mb-3">
          <label for="judul" class="form-label">Nama Product</label>
          <input type="text" class="form-control form-control-sm" name="judul" id="judul" aria-describedby="helpId" 
          placeholder="Nama Product" value="{{Session::get('judul')}}">

        </div>

        <div class="mb-3">
            <label for="fotoproduct" class="form-label">Foto</label>
          <input type="file" class="form-control form-control-sm" name="fotoproduct" id="fotoproduct" aria-describedby="helpId" 
